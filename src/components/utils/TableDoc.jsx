...
const TableDoc = ({ Props, children, title, subtitle, ifExist }) => {

    return (
        <div className="rounded-lg">
            {/* Título Principal */}
            <h1 className="text-3xl font-semibold text-center text-[#2fd484] mb-10">
                {title}
            </h1>
                ...
                <ul>
                    {Props.map((prop, index) => (
                        <li
                            ...
                            <div className="flex flex-col lg:w-1/2 text-gray-300 text-sm">{prop.description}</div>
                        </li>
                    ))}
                </ul>
                {/* Área para Children */}
